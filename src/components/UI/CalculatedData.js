...
const CalculatedData = ({ heading, value, unit, decimalCount}) => {
  return (
    <div>
      {value ? <p className="calculated-value">{!isNaN(decimalCount) ? formatValueWithTwoDecimals(value, decimalCount) : formatValueWithTwoDecimals(value)} {unit}</p> : <p className="pending-info">Pending Information</p>}
